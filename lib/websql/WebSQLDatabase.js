...
var getVersion = 'SELECT value FROM __WebKitDatabaseInfoTable__ ' +
    'WHERE key="WebKitDatabaseVersionKey";';   
var setVersion = 'INSERT OR REPLACE INTO __WebKitDatabaseInfoTable__ (key, value)' +
...
function SQLVerionError(message) {
  this.message = message;
  this.code = 2;
}
inherits(SQLVerionError, Error); //!?
...
function WebSQLDatabase(dbVersion, db, callback) {
  ...
  self._db.exec([createInfoTable], false, function (error) {
    ...
    self._db.exec([{ sql: getVersion }], true, function (error, res) {
      /* istanbul ignore next */
      if (error) {
        return self._callback(null, error);
      }
...
WebSQLDatabase.prototype.changeVersion = 
  ...
  self.transaction(function (tx) {
    tx.executeSql(getVersion, [], function (tx, res) {
      // Check existing verion
      /* istanbul ignore next */
      if (res.rows.length === 1) {
        var currentVersion = res.rows.item(0).value;
        if (oldVersion !== currentVersion) {
          return errorCallback(tx, new SQLVerionError(
      ...
      function updateVersion() {
        self.transaction(function (tx) {
          tx.executeSql(setVersion, [newVersion], function () {
